...
module Nested
  use utils1.f90
  use kmeans_clstr.f90
  use xmeans_clstr.f90
  use posterior.f90
  use priors.f90
  implicit none
...
!----------------------------------------------------------------------
  ...
  subroutine clusteredNest(p,phyP,l,loglike)
	...
	allocate( sc_npt(maxeCls), sc_mean(maxeCls,ndims), sc_tmat(maxeCls,ndims,ndims), sc_kfac(maxeCls), sc_eff(maxeCls), sc_vol(maxeCls) )
	...
	if(my_rank==0) then
		!memory allocation
		allocate(nptk(maxeCls), nptx(nlive), meank(maxeCls,ndims), &
	...
	do ff=1,maxIter
		...
		if(ic_done(0)) then
			if(my_rank==0) then
				...
				!memory deallocation
				deallocate(sc_invcov, sc_node, sc_eval, sc_evec)
			...
			deallocate( sc_npt, eswitchff, escount, dmin, evData, ic_sc, ic_npt, ic_done, ic_climits, &
...
!				endif
						...
						if(dino) then
							...
							do i3=1,50
								...
								.false.,ic_rFlag(i1),cSwitch,nCdims)) then
									...
									if(ceff) ic_eff(i,4)=ic_vnow(i1)*ic_volFac(i1)*dble(ic_npt(i1))/dble(nlive)/totVol(i1)
		...
		if(flag2 .and. eswitch) then
			...
			call MPI_BCAST(sc_npt(1:sc_n),sc_n,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
				...
				if(.not.eswitch) then
					...
					if(my_rank==0) then
						do
							...
							call selectEll(ic_sc(nd),sc_npt(nd_i+1:nd_i+ic_sc(nd)),i)
							i=i+nd_i
					...
					call MPI_BCAST(i,1,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
					call MPI_BCAST(sc_npt(i),1,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
					...
					call MPI_BCAST(sc_npt(q),1,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
			...
			if(eswitch) then
				...
				do i=1,ic_n
					if(ic_done(i)) then
						...
						if(ceff) ic_eff(i,4)=ic_vnow(i)*ic_volFac(i)*dble(ic_npt(i))/dble(nlive)/totVol(i)
					endif
					k=k+ic_sc(i)
				enddo
			endif
...
!---------------------------------------------------------------------- 
  ...
  subroutine selectEll(n,nptx,sEll)
 	
      implicit none
      
      integer n!total no. of ellipsoids
      integer nptx(n)!no. points in each ellipsoid
