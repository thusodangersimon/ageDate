...
class astro_filter(object):
	...
	def set_vega_conversion( self, vega ):
		...
		if type( vega ) != type( np.array( [] ) ) or vega.size == 0: raise ValueError( 'Pass a numpy array with the vega spectrum to calculate vega conversion!' )
	...
	def set_solar_magnitude( self, solar ):
		...
		if type( solar ) != type( np.array( [] ) ) or solar.size == 0: raise ValueError( 'Pass a numpy array with the solar spectrum to calculate the solar magnitude!' )
	...
	def grid( self, zf, vs, zs, ages, seds, force=False ):
		...
		self.store_grid( zf, zs, ages[w], rest, obs, dms )
