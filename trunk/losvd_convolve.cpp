...
void convolve(py::object  x, py::object y ,py::object losvd,py::object out){
  ...
  for (int n = 0; n < SignalLen; n++)
	...
	KernelLen = 2 * std::ceil(  5 * logl_simga) + 1;
	...
	for (int i = - HalfKernelLen  ; i <  KernelLen - HalfKernelLen; i++){
	  ...
	  if (param[2] != 0){
	    Kernel[j] *= (1 + (param[2] * 1.4142135623730951) / (2.449489742783178) * (2 * pow(Y[j],3) - 3 * Y[j]));
	  }
	  if (param[3] != 0){
	    Kernel[j] *= (1 + param[3] / (4.898979485566356) * (4 * pow(Y[j],4) - 12 * pow(Y[j],2) + 3));
	...
	i2 = KernelLen - 1;
      ...
      j=0;
      //std::cout << i1 <<" "<<i2<< " "<< KernelLen <<"\n";
      for (int i = i1; i <= i2; i++){
	temp_kern[j] = Kernel[i];
	norm += Kernel[i];
	//std::cout << i << " " <<  Kernel[i] <<' ' <<-1 <<' ';
	j++;
	}
      //std::cout <<"\n";
      j=0;
      for (int k = m1; k <= m2; k++){
	temp_con[j] = Signal[k] * temp_kern[j]/(norm-1);
	j++;
      }
